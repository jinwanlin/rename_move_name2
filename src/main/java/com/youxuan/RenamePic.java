...
public class RenamePic {
	public static void main(String[] args) throws IOException {
		...
		int lastDir = 15;
		String macDir = "/Users/jinwanlin/Desktop";
		
		String newDir = "/Users/jinwanlin/at/";
...
//		String dirPath = file.getAbsolutePath();// 目录路径
		if (file.isDirectory()) {
			...
			for (int i = 0; i < dirs.length; i++) {
				...
				if(dir.getName().charAt(0) == '.') continue;
				...
				if(dir.isDirectory()){
					...
					for (int j = 0; j < files.length; j++) {
						...
						if(!javaList.contains(exi)) continue;
						
						
						File newFile = new File(newDir+(i+lastDir+1)+"_"+j+exi);
						f.renameTo(newFile);
